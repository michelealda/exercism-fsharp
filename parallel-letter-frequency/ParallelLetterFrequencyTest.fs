...
[<Fact>]
let ``No texts mean no letters``() = frequency [] |> should be Empty

[<Fact>]
let ``One letter``() = frequency [ "a" ] |> should equal (Map.ofList [ ('a', 1) ])

[<Fact>]
let ``Case insensitivity``() = frequency [ "aA" ] |> should equal (Map.ofList [ ('a', 2) ])

[<Fact>]
let ``Many empty texts still mean no letters``() = frequency (List.replicate 10000 "  ") |> should be Empty

[<Fact>]
let ``Many times the same text gives a predictable result``() =
    frequency (List.replicate 1000 "abc")
    |> should equal
           (Map.ofList
               [ ('a', 1000)
                 ('b', 1000)
